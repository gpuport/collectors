...
class TestPipelineResult:
    ...
    def test_success_property_explicit_override(self) -> None:
        """Test explicit success overrides computed value."""
        from gpuport_collectors.export.pipeline import PipelineResult

        explicit_success = PipelineResult(
            pipeline_name="test",
            enabled=True,
            input_count=1,
            filtered_count=1,
        ...
        explicit_failure = PipelineResult(
            pipeline_name="test",
            enabled=True,
            input_count=1,
            filtered_count=1,
        ...
        assert explicit_failure.success is False
    ...
    def test_to_dict_includes_timing(self) -> None:
        ...
        assert data["filter_duration"] == 0.1
        assert data["transform_duration"] == 0.2
        assert data["output_duration"] == 0.3
...
class TestExecutePipeline:
    ...
    def test_unknown_transformer_type(self, sample_instances: list[GPUInstance]) -> None:
        """Test pipeline failure on unknown transformer type."""
        config = PipelineConfig(
            name="unknown-transformer",
            transformer=JSONTransformerConfig(),
            outputs=[],
        )
        config.transformer = object()  # type: ignore[assignment]

        result = execute_pipeline(sample_instances, config)

        assert result.success is False
        assert result.error is not None
        assert "Unknown transformer type" in result.error

    def test_s3_and_https_outputs(self, sample_instances: list[GPUInstance]) -> None:
        """Test S3 and HTTPS outputs are reported correctly."""
        ...
        config = PipelineConfig(
            ...
            transformer=JSONTransformerConfig(),
            outputs=[
                S3OutputConfig(bucket="test-bucket", prefix="exports"),
                HTTPSOutputConfig(url="https://example.com/webhook"),
            ],
        )

        with (
            patch("gpuport_collectors.export.pipeline.write_to_s3", return_value="file.json"),
            patch(
                "gpuport_collectors.export.pipeline.write_to_https",
                return_value={
                    "total_requests": 1,
        ...
        assert result.success is True
        assert result.output_count == 2
        assert result.outputs[0]["type"] == "s3"
        assert result.outputs[0]["bucket"] == "test-bucket"
        assert result.outputs[0]["key"] == "file.json"
        assert result.outputs[1]["type"] == "https"
        assert result.outputs[1]["url"] == "https://example.com/webhook"
        assert result.outputs[1]["total_requests"] == 1

    def test_unknown_output_type(self, sample_instances: list[GPUInstance]) -> None:
        """Test unknown output type is tracked as failure."""
        ...
        assert result.output_count == 1
        assert result.outputs[0]["success"] is False
        assert result.outputs[0]["type"] == "custom"
        assert "Unknown output type" in result.outputs[0]["error"]
