...
from gpuport_collectors.config import CollectorConfig, HttpClientConfig, default_config
...
class TestCollectorConfig:
    ...
    def test_timeout_validation_positive(self):
        ...
        with pytest.raises(ValidationError, match="greater than 0"):
            CollectorConfig(http_client=HttpClientConfig(timeout=0))

        with pytest.raises(ValidationError, match="greater than 0"):
            CollectorConfig(http_client=HttpClientConfig(timeout=-1))
    ...
    def test_timeout_validation_max(self):
        ...
        with pytest.raises(ValidationError, match="cannot exceed 300 seconds"):
            CollectorConfig(http_client=HttpClientConfig(timeout=301))
    ...
    def test_max_retries_validation(self):
        ...
        with pytest.raises(ValidationError, match="greater than or equal to 0"):
            CollectorConfig(http_client=HttpClientConfig(max_retries=-1))

        # Should reject values > 10
        with pytest.raises(ValidationError, match="cannot exceed 10"):
            CollectorConfig(http_client=HttpClientConfig(max_retries=11))

    def test_backoff_factor_validation(self):
        """Test that backoff_factor must be positive."""
        with pytest.raises(ValidationError, match="greater than 0"):
            CollectorConfig(http_client=HttpClientConfig(backoff_factor=0))

        with pytest.raises(ValidationError, match="greater than 0"):
            CollectorConfig(http_client=HttpClientConfig(backoff_factor=-1))
    ...
    def test_model_dump(self):
        ...
        # Now nested under http_client
        assert data["http_client"]["timeout"] == 60
        assert data["http_client"]["max_retries"] == 5
        assert data["http_client"]["backoff_factor"] == 1.5
        assert data["http_client"]["base_delay"] == 5.0
